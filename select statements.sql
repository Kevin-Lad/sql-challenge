-- Select statements

--List the employee number, last name, first name, sex, and salary of each employee
SELECT e.emp_number, e.last_name, e.first_name, e.sex, s.salary
FROM employees e
JOIN salaries s
ON e.emp_number = s.emp_number;

--List the first name, last name, and hire date for the employees who were hired in 1986
SELECT first_name, last_name, hire_date
FROM employees
WHERE hire_date > ('12/31/1985') AND hire_date < ('1/1/1987');

--List the manager of each department along with their department number, department name, 
--employee number, last name, and first name
SELECT dm.dept_number, d.dept_name, e.emp_number, e.last_name, e.first_name
FROM employees e
JOIN department_mgr dm
ON e.emp_number = dm.emp_number
JOIN departments d
ON d.dept_number = dm.dept_number;

--List the department number for each employee along with that employee’s employee number, last name, 
--first name, and department name
SELECT de.dept_number, e.emp_number, e.last_name, e.first_name, d.dept_name
FROM employees e
JOIN department_emp de
ON e.emp_number = de.emp_number
JOIN departments d
ON d.dept_number = de.dept_number;

--List first name, last name, and sex of each employee whose first name is Hercules and whose last name 
--begins with the letter B
SELECT first_name, last_name, sex
FROM employees
WHERE first_name = 'Hercules'
AND last_name LIKE 'B%';

--List each employee in the Sales department, including their employee number, last name, and first name
SELECT d.dept_name, e.emp_number, e.last_name, e.first_name
FROM employees e
JOIN department_emp de
ON e.emp_number = de.emp_number
JOIN departments d
ON d.dept_number = de.dept_number
WHERE d.dept_name = 'Sales';

--List each employee in the Sales and Development departments, including their employee number, last name, 
--first name, and department name
SELECT e.emp_number, e.last_name, e.first_name, d.dept_name
FROM employees e
JOIN department_emp de
ON e.emp_number = de.emp_number
JOIN departments d
ON d.dept_number = de.dept_number
WHERE d.dept_name IN ('Sales', 'Development');

 
--List the frequency counts, in descending order, of all the employee last names (that is, how many employees 
--share each last name)
SELECT last_name, COUNT(last_name) AS frequency
FROM employees
GROUP BY last_name
ORDER BY frequency DESC
